...
for (const code of codes) {
  ...
  // Anything already translated, and anything the catalogue has decided is deliberately identical.
  // Without the second set those strings are paid for on every run and never satisfied: the model
  // correctly echoes "DPI —" back, the echo guard below correctly refuses to store an echo, and the
  // next run asks again. A term that is meant to stay English is finished, not missing.
  const settled = new Set(existing.same_as_english || []);
  let pending = inventory.filter((s) => !strings[s] && !settled.has(s));
  ...
  for (let i = 0; i < pending.length; i += batchSize) {
    ...
    // Spread `existing` first so nothing a human put in the file is dropped on the way through.
    // `same_as_english` was: it is where a catalogue records that a term is deliberately identical
    // ("Graphic Novels" in German, "Psalm 46:10" in Dutch, "DPI —" in all fifteen), and rebuilding
    // the object from a fixed field list deleted it on the very first batch. Those strings then came
    // back as missing, so the next run paid to have a product name and a Bible citation
    // "translated", and the audit's echo check started reporting the deliberate ones as faults.
