...
(async () => {
  const { prompt, cookie, outdir, profile } = parseArgs();
  if (!prompt || !outdir) {
  ...
  try {
    ...
    if (inputHandle) {
      ...
      console.log(
        "No obvious input element found. Please paste the prompt into the Midjourney UI and send it manually. Waiting for images...",
      );
    }
