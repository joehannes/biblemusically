  ...
  try {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      authCookie = cookies.find((c) =>
        /next-auth.session-token|__Secure-next-auth.session-token|session-token|midjourney|mj_session/i.test(
          c.name,
        ),
      );
      if (authCookie) break;
    ...
    if (!authCookie) {
      const error = {
        ...
        error: "timeout",
        detail:
          "No Midjourney authentication cookie was detected within the timeout. Please complete the login manually in the opened browser.",
        url: page.url(),
      };
      await context.close();
