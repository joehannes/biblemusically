...
(async () => {
  ...
  try {
    ...
    while ((Date.now() - start) / 1000 < timeout) {
      ...
      const found = cookies.find(c => c.name === 'studio-api_key' || c.name === 'studio-api_key_local');
      if (found && found.value) {
        // Print JSON to stdout
        console.log(JSON.stringify({ ok: true, cookie: `${found.name}=${found.value}` }));
        await browser.close();
        process.exit(0);
      }
      // small delay
    ...
    await browser.close();
    console.error(JSON.stringify({ ok: false, detail: 'timeout waiting for Suno login/cookie' }));
    process.exit(2);
  } catch (e) {
    try { await browser.close(); } catch (_) {}
    console.error(JSON.stringify({ ok: false, detail: e && e.message ? e.message : String(e) }));
    process.exit(1);
  }
})();
