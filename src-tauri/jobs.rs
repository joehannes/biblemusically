    ...
    for _ in 0..40 {
        ...
        if let Some(arr) = clips.as_array() {
            ...
            if let Some(clip) = ready.first() {
                let dur = clip["metadata"]["duration"].as_f64().unwrap_or(120.0);
...
async fn real_ffmpeg(
    ...
    if which::which(ff).is_err() {
        db_log(db, job_id, &format!("ffmpeg: '{}' not found, using mock", ff)).await;
        return None;
    }
    ...
    if images.is_empty() || audio_url.is_none() {
        db_log(db, job_id, "ffmpeg: missing images or audio_url, using mock").await;
        return None;
    }
    let client = reqwest::Client::new();
    for (i, url) in images.iter().enumerate() {
        if let Ok(res) = client.get(*url).send().await {
            if let Ok(bytes) = res.bytes().await {
                let _ = tokio::fs::write(out_dir.join(format!("img_{i:03}.jpg")), bytes).await;
            }
        }
    }
    if let Ok(ar) = client.get(audio_url.unwrap()).send().await {
        if let Ok(bytes) = ar.bytes().await {
            let _ = tokio::fs::write(out_dir.join("audio.mp3"), bytes).await;
        }
    }
    ...
    let status = tokio::process::Command::new(ff)
        .args(["-y", "-framerate", &format!("1/{per}"),
               "-i", out_dir.join("img_%03d.jpg").to_str().unwrap(),
               "-i", out_dir.join("audio.mp3").to_str().unwrap(),
    ...
    db_log(db, job_id, "ffmpeg: compose failed").await;
    None
}
    ...
    let run_result: anyhow::Result<Value> = async {
        Ok(match kind {
            ...
            "music" => {
                ...
                let (audio_url, duration, is_real) = if let Some(r) = real {
                    (r["audio_url"].as_str().unwrap_or("").to_string(),
                     r["duration"].as_f64().unwrap_or(120.0), true)
                } else {
                    let mut rng = rand::thread_rng();
                    (format!("https://cdn.suno.ai/mock/{tgt}.mp3"),
                     92.0 + rng.gen::<f64>() * 30.0, false)
                };
                db.collection::<Document>("songs").update_one(
                    doc! { "id": tgt },
                    doc! { "$set": { "audio_url": &audio_url, "duration": duration, "status": "music_ready" } },
                ).await?;
                serde_json::json!({ "audio_url": audio_url, "real": is_real })
            }
            ...
            "image" => {
                ...
                let prompt = sec["image_prompt"].as_str()
                    ...
                    .unwrap_or("")
                    .to_string();
                let variants = if let Some(v) = real_mj(&prompt, &settings_doc, job_id, db).await {
                    db_log(db, job_id, "mj: real images received").await;
                    v
                } else {
                    let seed = prompt.bytes().map(|b| b as u64).sum::<u64>() % 10000;
                    (0..4).map(|i| format!("https://picsum.photos/seed/{}/1280/720", seed + i)).collect()
                };
                db.collection::<Document>("sections").update_one(
                    doc! { "id": tgt },
                    doc! { "$set": {
                        "image_url": &variants[0],
                        "image_variants": variants.iter().map(|s| bson::Bson::String(s.clone())).collect::<Vec<_>>()
                    }},
                ).await?;
                serde_json::json!({ "variants": 4 })
            }
