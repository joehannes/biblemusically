...
import { Wand2, Layers, Grid3x3, List as ListIcon, Columns } from "lucide-react";
...
export default function Images() {
  ...
  useEffect(() => { load(); const t = setInterval(load, 3000); return () => clearInterval(t); }, [activeSongId]);

  const gen = async (id) => { await api.genImage(id); toast.success("Image queued"); };
  const batch = async () => { if (!activeSongId) return; const r = await api.batchImages(activeSongId); toast.success(`Queued ${r.queued}`); };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto fade-in">
      <div className="text-mono text-[11px] uppercase tracking-[0.3em] text-muted-foreground mb-2">step 6</div>
      ...
      <p className="text-muted-foreground mb-6 max-w-2xl">Sent to Midjourney via session cookie or Discord wrapper. Mix presets &amp; tweak params before generating.</p>

      <Card className="p-5 mb-6 grid md:grid-cols-6 gap-4 items-center">
        <Select value={activeSongId || ""} onValueChange={selectSong}>
          <SelectTrigger data-testid="images-song-select" className="md:col-span-2"><SelectValue placeholder="Song" /></SelectTrigger>
      ...
      <div className={view==="grid"?"grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3" : view==="column" ? "columns-2 md:columns-3 gap-3 space-y-3" : "space-y-3"}>
        {sections.map(s => {
          ...
          return (
            <Card key={s.id} data-testid={`image-card-${s.index}`} className={`overflow-hidden ${view==="list"?"flex gap-3":""}`}>
              <div className={`relative ${view==="list"?"w-48 shrink-0":"w-full"} ${aspect} bg-muted/30 ${view==="list"?"":"rounded-t-md"} overflow-hidden`}>
                {img ? <img src={img} alt="" className="w-full h-full object-cover" /> : <div className="absolute inset-0 flex items-center justify-center text-xs text-muted-foreground text-mono">no image</div>}
              ...
              <div className="p-3 flex-1 min-w-0">
                ...
                <div className="flex gap-1">
                  <Button size="sm" variant="secondary" data-testid={`image-gen-${s.index}`} onClick={()=>gen(s.id)}><Wand2 className="w-3 h-3 mr-1" />{img?"Vary":"Gen"}</Button>
                </div>
              </div>
            </Card>
          );
