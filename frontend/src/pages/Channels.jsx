...
import { Tv, Plus, Link as LinkIcon, Trash2, ShieldCheck } from "lucide-react";
...
export default function Channels() {
  ...
  const startOauth = async (c) => {
    const r = await api.oauthStart(c.id);
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto fade-in">
      ...
      <p className="text-muted-foreground mb-6 max-w-2xl">Add YouTube channels and connect each via Google OAuth using the client credentials you configured in Settings.</p>
      ...
      <Dialog open={!!oauthDialog} onOpenChange={(v)=>!v && setOauthDialog(null)}>
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">Authorize in the opened tab, then paste the refresh token (and optionally channel info) here.</p>
          <Input data-testid="oauth-refresh-input" placeholder="refresh_token" value={refresh} onChange={e=>setRefresh(e.target.value)} />
          ...
          <Button data-testid="oauth-complete-btn" onClick={completeOauth}><LinkIcon className="w-4 h-4 mr-2" />Save</Button>
        </DialogContent>
      </Dialog>
    </div>
  );
